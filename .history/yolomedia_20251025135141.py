...
这是AI智能眼镜系统的物品查找模块，负责：
1. 物品识别：使用YOLO-E开放词汇检测目标物品
2. 手部追踪：结合MediaPipe手部检测，引导用户手部靠近物品
3. 抓取检测：检测手部握持动作，确认物品已拿到
4. 多模态反馈：视觉标注+语音引导+居中提示
...
工作模式：
- SEGMENT: 检测模式（寻找目标物品）
- FLASH: 闪烁确认（找到物品时的视觉反馈）
- CENTER_GUIDE: 居中引导（引导手部到物品中心）
- TRACK: 手部追踪（追踪手部移动）
...
def main(headless: bool = False, prompt_name: str = None, stop_event=None):
    ...
    参数：
    - headless: 是否无头模式运行（不显示GUI）
    - prompt_name: 目标物品名称（英文类别名）
